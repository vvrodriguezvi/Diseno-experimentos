...
Posteriormente, se compró en un almacén del _D1_ la sustancia a ser echada en la tela, en este caso se optó por el vino, puesto que se concluyó que con estos cuatro tipos de tela blanca la mancha sería muy visible, en consecuencia esto permite que a la hora de realizar la calificación por cada uno de los integrantes la mancha sea lo que más resalte de la tela, ya sea por su ausencia o presencia de la misma después de aplicar el tratamiento (detergente líquido). Después se compraron las tres distintas marcas de detergente líquido también en un establecimiento _D1_ los cuales fueron _Bonaropa_ de 1000 ml costo 3.890 pesos, _Limpido_ de 1000 ml costo 4.328 pesos y _Vanish_ de 1000 ml costo 10.278 pesos.

Adicionalmente, se utilizaron materiales secundarios para la realización de dicho experimento, materiales tales como: 2 tarros para muestras de orina, tijeras, marcadores, lápiz, cinta de enmascarar,, vasos transparentes desechables y un gotero. Ya después de haber conseguido estos materiales se procedió a realizar la muestra piloto, dicho procedimiento se detalla seguidamente.


\subsubsection{Prueba piloto}

Se realizó una prueba piloto con el fin de identificar qué inconvenientes se pueden llegar a presentar al momento de realizar el experimento final. El diseño piloto se llevó a cabo de la siguiente manera: de las 20 divisiones de cada tipo de tela se escogió de manera aleatoria 3 retazos de tela para cada uno de los 3 diferentes tipos de detergentes, estos retazos fueron cortados y luego manchados cada uno con 3 gotas de vino y se esperó un lapso de 1 minuto para que la mancha se acentuará sobre la tela y luego se procedió a dejar  el retazo de tela remojando en la solución de agua con cada uno de los detergentes durante dos minutos.
...
Para el proceso de aleatorización se llevo a cabo lo siguiente:

__1.__ Para cada uno de los tipos de tela se realizó una división de 20 fragmentos de 10x10cm.

__2.__ Se le asignó un número a cada una de las 20 divisiones en cada uno de los diferentes tipos de tela.

__3.__ Se pusieron en un vaso desechable 20 papelitos con el números del 1 a 20, de esos se escogieron 3 números aleatorios, para seleccionar qué división de la tela fue cortada y manchada con vino, donde el primer número aleatorio corresponde al retazo de tela que fue sumergido en la solución de agua y el detergente _Bonaropa_, el segundo número aleatorio corresponde al retazo de tela que fue sumergido en la solución de agua y el detergente _Límpido_. El tercer número aleatorio corresponde al retazo de tela que fue sumergido en la solución de agua y el detergente _Vanish_.

__4.__ Este proceso anterior se repitió con cada una de las diferentes tipos de tela.


\subsection{Aplicación del diseño experimental}

Después de haber obtenido las unidades experimentales producto de la aleatorización. Se realizaron 3 réplicas del diseño factorial para investigar el efecto de las telas y de los detergentes en la eficiencia al eliminar una mancha de vino. Esencialmente, se les aplicó a las telas tres gotas de vino, utilizando un gotero y velando porque el perímetro de las manchas para cada tipo de tela fuera el más similar posible. Se esperó un tiempo prudencial de cinco minutos, tiempo en el cual el vino ya se había secado en todas las telas.

Posteriormente, se sumergió las telas en vasos desechables que contenían agua y el detergente en cuestión, específicamente se echaron dos basados de agua por uno de detergente usando como medidor un frasco para muestras de orina de 56.3 mm. Se esperó un tiempo de aproximadamente cinco minutos, entre cada tanda de telas, es decir, las tres corridas de tela distintas por tela y detergente. También, se registraron los órdenes en que fueron tomadas las mediciones.

Finalmente, se extrajeron las telas y se escurrieron, luego se organizaron preservando el orden en el cual se habían aplicado los tratamientos a la tela. Seguidamente, cada integrante del equipo de manera individual observaba cada una de las telas y realizaba la calificación. Así mismo para cada una de las tandas o corridas experimentales realizadas.

Los promedios obtenidos por cada detergente y tipo de tela en cada réplica aparece en la tabla siguiente:
...
\begin{cases}
\begin{aligned}
H_0&: \varepsilon_{ijk} \sim  Normales \\
H_a&: \varepsilon_{ijk} \nsim Normales\ 
\end{aligned}
\end{cases}
$$
...
Según los resultados anteriores, se quiere responder a la pregunta de cúal de los detergente o tipos de tela tiene una mejor calificación al momento de eliminar una mancha de vino, y como se puedo ver el los gráficos de los efectos, se puede observar que el tipo de detergente Límpido presentaba un puntaje de calificación más alto con respecto a los demás detergentes; y con respecto a las telas, se observaba que el tipo de tela poliéster presenta menores puntajes de eficiencia. Para corroborar lo anterior se plantean diversas pruebas de contrastes, pero antes de esto, se presentan algunos gráficos que permiten corroborar los resultados

1. Boxplot para el factor A: Detergente

```{r, fig.cap="Box-plot para tipo de Detergente"}
...
- Del gráfico se puede observar que el tipo de Detergente Límpido es quién presenta menor variabilidad, y el tipo de Detergente Vanish es quién presenta una variablidad mucho mayor con respectos a los otros dos tipos de detergentes. 

- Del Box-Plot se podría inferir que existe diferencia significativa entre las medias de los promedios  de la eliminación de una mancha de vino según el tipo de Detergente, ya que para cada una de las 3 cajas del gráfico, se observan comportamientos diferentes.

2. Boxplot para el factor B: Tipo de tela

```{r, fig.cap="Box-plot para tipo de Tela"}
...
- Del gráfico se puede observar que el tipo de Tela Poliéster es quién presenta mayor variabilidad, y el tipo de Tela Lino y Microfibra presentan menor variablidad respectos a los otros dos tipos de telas. 

- Del Box-Plot se podría inferir que existe diferencia significativa entre las medias de los promedios  de eficiencia de la eliminación de una mancha de vino según el tipo de tela, ya que para cada una de las 4 cajas del gráfico, se observan comportamientos diferentes.


Para la diferencia de medias Se procede a utilizar el método Duncan para comparaciones de medias:

\subsection{Diferencia de medias del Factor A}

Para determinar si existe diferencia significativa entre las medias según el tipo de Detergente, se platea la siguiente prueba de hipótesis:
...
\subsection{Diferencia de medias del Factor B}

Para determinar si existe diferencia significativa entre las medias según el tipo de Tela, se platea la siguiente prueba de hipótesis:
...
El diseño experimental presentado tuvo el fin de dar a conocer la eficiencia de tres marcas de detergentes (_Limpido_, _Bonaropa_ y _vanish_) a la hora de eliminar o hasta cierto punto mitigar una mancha causada por el vino en cuatro distintos tipos de tela (lino, _scuba_, poliester y microfibra). A continuación, se presentan conclusiones, observaciones, comentarios y recomendaciones; dadas por los investigadores y que son fruto del desarrollo de este experimento, así como los resultados e impresiones generadas durante la realización del mismo.

Del diseño experimental es menester destacar lo siguiente:

- El __Lmpido__ en definitiva, resulto ser el que presento mejores resultados a la hora de eliminar la mancha de vino, ya que sin importar el tipo de tela, esta marca dejo, en la mayoría de los casos, nula evidencia de la mancha de vino.

- Resulta curioso dar cuenta de que, el detergente __Vanish__ pese a ser el más costoso de todos, fue el que menos resultados mostró a la hora de mitigar la mancha de vino. Sin embargo, resulta ser fácil de manipular y no deterioro tanto las telas a diferencia del detergente __limpido__.

- Se observó en cuanto a las telas _scuba_ y microfibra, como en este era un poco más complicado que la sustancia del vino se diluyera en ellas y perdurara dado la contextura de las mismas.

- El lino es el tipo de tela que presentó menor resistencia a la remoción de la mancha del vino, al aplicar cualquiera de los 3 detergentes.

- El poliéster fue el tipo de tela que mejor absorbió la mancha de vino, por lo que el detergente _Bonaropa_ y el detergente _Vanish_ demostraron ser menos efectivos en la remoción de la mancha de vino en este tipo de tela.
